...
export default function DeckCard({ deck, onDeckClick, onDeckDeleted, onDeckUpdated, index }: DeckCardProps) {
  ...
  if (isEditing) {
    return (
      <div
        className="relative bg-white/80 backdrop-blur-sm rounded-2xl border border-white/20 shadow-lg transition-all duration-300 overflow-hidden p-6"
        style={{
      ...
      >
        <h3 className="text-xl font-bold text-slate-800 mb-4">Renommer le deck</h3>
        <input
          type="text"
          value={newDeckName}
          onChange={(e) => setNewDeckName(e.target.value)}
          className="w-full px-4 py-3 bg-white/70 border border-slate-300/50 rounded-lg text-lg font-medium text-slate-800 placeholder-slate-500 focus:outline-none focus:bg-white/90 focus:ring-2 focus:ring-blue-500 transition-all duration-300"
        ...
        <div className="mt-4 flex justify-end gap-3">
          <button 
            ...
            className="font-semibold text-slate-600 py-2 px-5 rounded-full hover:bg-slate-500/10 transition-colors duration-300"
          >
            Annuler
          </button>
          <button 
  ...
  return (
    <div
      ...
      className="group relative bg-white/80 backdrop-blur-sm rounded-2xl border border-white/20 shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 cursor-pointer overflow-hidden"
      style={{
        animationName: 'fadeInUp',
        animationDuration: '0.6s',
        animationTimingFunction: 'ease-out',
      ...
      <div className="relative z-10 p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            <h3 
              className="text-xl font-bold text-slate-900 group-hover:text-blue-900 transition-colors duration-300 line-clamp-2"
              title={deck.name}
            >
              {deck.name}
            </h3>
          ...
          <Menu as="div" className="relative ml-3">
            <div>
              <Menu.Button 
                className="flex items-center p-2 rounded-full hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <Menu.Items className="absolute right-0 mt-2 w-48 origin-top-right bg-white rounded-lg shadow-xl z-20 border border-gray-200 focus:outline-none">
              <Menu.Item>
                {({ active }: { active: boolean }) => (
                  <button
                    onClick={handleRenameClick}
                    className={`${active ? 'bg-gray-100' : ''} block w-full text-left px-4 py-2.5 text-sm text-gray-700 rounded-t-lg`}
                  >
                    Renommer
                  </button>
                )}
              ...
              <Menu.Item>
                {({ active }: { active: boolean }) => (
                  <button
                    onClick={openConfirm}
                    className={`${active ? 'bg-red-50' : ''} block w-full text-left px-4 py-2.5 text-sm text-red-600 rounded-b-lg`}
                  >
                    Supprimer
                  </button>
                )}
              ...
              >
                <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                  ...
                  <div className="mt-4 flex justify-end gap-2">
                    ...
                    <button
                      type="button"
                      className="inline-flex justify-center px-4 py-2 text-sm font-medium text-red-600 bg-red-100 border border-transparent rounded-md hover:bg-red-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-red-500"
        ...
        <div className="space-y-3">
          {deck.pdf_file_name && (
            <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 px-3 py-2 rounded-lg">
              ...
              <span className="font-medium line-clamp-2" title={deck.pdf_file_name}>{deck.pdf_file_name}</span>
